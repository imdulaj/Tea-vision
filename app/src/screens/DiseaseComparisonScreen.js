...
const DiseaseComparisonScreen = ({ route, navigation }) => {
  ...
  if (analysisResult?.classification) {
    ...
    predictedClass =
      diseaseName || analysisResult?.predicted_class || "Unknown";
    confidence =
      percentage ||
      (analysisResult?.confidence_percent
        ? parseFloat(analysisResult.confidence_percent).toFixed(1)
        : "0.0");
  }

  // Determine which comparison card is the "primary" match based on prediction
  const isRedRust =
    predictedClass.toLowerCase() === "red_rust" ||
    predictedClass.toLowerCase() === "red rust" ||
    predictedClass.toLowerCase() === "redrust";
  const isPotassium =
    predictedClass.toLowerCase() === "potasium-k" ||
    predictedClass.toLowerCase() === "potassium_deficiency";

  // Map the actual classification probabilities to the "Other diseases" view, showing those with low confidence
  const allProbabilitiesList = Object.keys(probabilities)
    .map((key) => ({
    ...
    .filter(
      (item) =>
        item.name.toLowerCase() !== predictedClass.toLowerCase() &&
        item.name.toLowerCase() !== "healthy",
    )
    .sort((a, b) => b.prob - a.prob)
    .slice(0, 5) // Display top 5 other low-probability matches
    .map(
      (item) =>
        `${item.name.charAt(0).toUpperCase() + item.name.slice(1)} (${item.prob}%)`,
    );

  const otherDiseases =
    allProbabilitiesList.length > 0
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <View style={styles.toggleContainer}>
          ...
          >
            <Text
              style={[styles.toggleText, !showProcessed && styles.activeText]}
            >
              Original
            </Text>
          </TouchableOpacity>
        ...
        <View style={styles.otherDiseasesContainer}>
          <Text style={styles.sectionHeader}>
            Other Potential Diseases Evaluated
          </Text>
          <Text style={styles.instructionDesc}>
            The system also checked for the following but found low probability:
          </Text>
            ...
            <>
              <View style={styles.instructionItem}>
                ...
                <View style={styles.instructionTextContent}>
                  ...
                  <Text style={styles.instructionDesc}>
                    Apply 50% Copper Oxychloride fungicide directly onto
                    affected areas to control spread.
                  </Text>
                </View>
              </View>
              ...
              <View style={styles.instructionItem}>
                ...
                <View style={styles.instructionTextContent}>
                  ...
                  <Text style={styles.instructionDesc}>
                    Ensure adequate pruning is performed to improve air
                    circulation and reduce trapped humidity.
                  </Text>
                </View>
              </View>
            </>
            ...
            <>
              <View style={styles.instructionItem}>
                ...
                <View style={styles.instructionTextContent}>
                  ...
                  <Text style={styles.instructionDesc}>
                    Apply 60kg/ha of MOP fertilizer to the soil.
                  </Text>
                </View>
              </View>
              ...
              <View style={styles.instructionItem}>
                ...
                <View style={styles.instructionTextContent}>
                  ...
                  <Text style={styles.instructionDesc}>
                    Ensure adequate irrigation to help nutrient absorption.
                  </Text>
                </View>
              </View>
            </>
