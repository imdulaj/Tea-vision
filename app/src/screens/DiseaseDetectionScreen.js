...
const DiseaseDetectionScreen = ({ navigation }) => {
  ...
  const handleCameraCapture = async () => {
    if (cameraRef.current) {
      try {
        console.log(
          `[${ts()}] INFO  DiseaseDetector  Capturing photo from camera...`,
        );
  ...
  const analyzeImage = async (uri) => {
    ...
    try {
      ...
      console.log(
        `[${ts()}] INFO  DiseaseDetector  Requesting segmentation from /predict`,
      );
      await delay(1000);

      setLoadingStep("Classifying disease patterns...");
      console.log(
        `[${ts()}] INFO  DiseaseDetector  Requesting classification from /classify-leaf`,
      );
      ...
      console.log(
        `[${ts()}] DEBUG DiseaseDetector  Segmentation:`,
        JSON.stringify(segResult).substring(0, 100) + "...",
      );
      console.log(
        `[${ts()}] DEBUG DiseaseDetector  Classification:`,
        JSON.stringify(clsResult),
      );
      ...
      if (segResult.status === "success" || clsResult.success) {
        ...
        console.log(
          `[${ts()}] INFO  DiseaseDetector  Analysis complete. Result: ${finalResult.classification?.predicted_class || "Unknown"}`,
        );
      } else {
        console.log(`[${ts()}] WARN  DiseaseDetector  Server reported failure`);
        Alert.alert(
          "Analysis Failed",
          "Could not classify the image properly.",
        );
      }
    } catch (error) {
      console.error(
        `[${ts()}] ERROR DiseaseDetector  Network/API error:`,
        error,
      );
    ...
    return (
      <SafeAreaView style={[styles.container, styles.centered]}>
        ...
        <Text style={styles.permissionText}>
          Camera access needed for disease detection
        </Text>
        <TouchableOpacity
        ...
        >
          <View style={styles.cameraOverlay}>
            ...
            <Text style={styles.cameraTip}>
              Point at a single leaf for best results
            </Text>
          </View>
        </CameraView>
      </View>
            ...
            <View style={styles.tipsSection}>
              ...
              <View style={styles.tipItem}>
                ...
                <Text style={styles.tipText}>
                  Keep leaf centered and in focus
                </Text>
              </View>
              <View style={styles.tipItem}>
                <CheckCircle color={COLORS.primary} size={16} />
            ...
            {!isLoading &&
              ...
              (() => {
                ...
                const hasLeaf = keys.some(
                  (key) => key.toLowerCase() === "leaf",
                );
                const hasDisease = keys.some(
                  (key) => key.toLowerCase() !== "leaf",
                );
                    ...
                    <View style={styles.errorResultCard}>
                      ...
                      <View style={{ marginLeft: 15, flex: 1 }}>
                        ...
                        <Text style={styles.errorDesc}>
                          We couldn't clearly identify a tea leaf or disease.
                ...
                //find disease percentage
                const percentage =
                  totalPixels > 0
                    ? ((maxPixels / totalPixels) * 100).toFixed(1)
                    : 0;
                const formattedDiseaseName =
                  diseaseName === "potasium-K"
                  ...
                  <View style={styles.resultContainer}>
                        ...
                        >
                          <Text
                            style={[
                              ...
                              { color: isCritical ? "#C62828" : "#2E7D32" },
                            ]}
                          >
                    ...
                    <View style={styles.treatmentCard}>
                      ...
                      <TouchableOpacity
                        ...
                        onPress={() =>
                          navigation.navigate("DiseaseComparison", {
                      ...
                      >
                        ...
                        <Text style={styles.comparisonBtnText}>
                          View details
                        </Text>
                      </TouchableOpacity>
                    </View>
